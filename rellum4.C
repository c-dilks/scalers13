...
{
  const Bool_t USE_RSC=1; // if true, uses rate-safe counts rather than CDF multiples corrections to compute rellum
  // -- NOTE if this is set to true, then the E,W,X distributions for rellum will be the same, since
  //    RSC only gives one value, combining E,W,X
